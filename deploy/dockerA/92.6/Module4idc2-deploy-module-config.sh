...
APPDIR="$BASEDIR/module"
APPTARBALL="$BASEDIR/module*.war"
...
CONFIDC=$APPDIR/WEB-INF/spring/dubbox.properties
CONFIDCBAK=$CONFIDC.bak
cat $CONFIDC | sed 's/^dubbo.protocol.host=mgt/dubbo.protocol.host=mgt2/g'> $CONFIDCBAK
...
function get_pid {
  ...
  PID=`ps -ef | grep "$TOMCAT_BASEDIR/endorsed" | grep -v grep | awk '{print $2}'`
