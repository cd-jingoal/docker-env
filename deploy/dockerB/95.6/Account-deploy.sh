...
if [[ ! -e $BASEDIR/tmp/message-bridge-demo/ ]]; then
	mkdir -p $BASEDIR/tmp/message-bridge-demo/
fi
...
function get_pid {
  ...
  PID=`ps -ef | grep "com.jingoal.account.server.SlaveServer" | grep -v grep | awk '{print $2}'`
...
function start {
  ...
  echo "call startSlave.sh ..."
  cd $APPDIR
  ./startSlave.sh > /dev/null &
  ...
  if [[ ! $PID ]]; then
    error "startSlave.sh called, but can't find process."
  fi
