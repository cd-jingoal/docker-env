...
BASEDIR="/usr/local/mgt_shadow"
APPDIR="$BASEDIR/shadow-sync"
APPTARBALL="$BASEDIR/shadow-sync-*.tar.gz"
BINDIR="$APPDIR/bin"
...
sed -i s/.test2.com/-cd.test1.com/g `find $APPDIR | grep .*[.]properties$`
sed -i s/.cd.test1.com/-cd.test1.com/g `find $APPDIR | grep .*[.]properties$`
...
# stop
./stop.sh || (echo "run stop.sh failed."; exit 1;)
